...
export default function Header( {}: HeaderProps) {
    ...
    return (
        <header className="w-full h-40 bg-[linear-gradient(to_left,rgba(220,38,38,1),rgba(220,38,38,0)),url('/header.jpg')] bg-cover bg-center flex items-center justify-between pr-10 py-4 shadow-lg">
            ...
            <div className="flex  gap-8 items-center"><button type="button" className="text-white font-bold ">{t("auth.login")}</button>
                <button type="button" onClick={handleOpenModal} className="text-white font-bold text-nowrap items-center justify-center px-4 h-12 rounded-lg bg-red-400 hover:drop-shadow-[0_0_6px_white] transition-all duration-300" >{t("auth.signup")}</button>
