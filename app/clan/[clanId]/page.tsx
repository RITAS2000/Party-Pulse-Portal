...
import { fetchClanMessage, getClans } from '@/redux/clan/operation';
...
const currentClan = useMemo(() => {
  ...
  useEffect(() => {
  if (clanId) {
    dispatch(fetchClanMessage(clanId));
  }
}, [dispatch, clanId, ]);
  ...
  <main className="pl-80 pt-40 min-w-screen min-h-screen flex flex-col"
    ...
    {isOpen && clanId && (
      ...
      <div className=' flex flex-row  '>  
        ...
        <AddToClanForm currentClan={currentClan} /><div className='w-full pb-16 border-l border-solid border-gray-700'>или нет персонажей для вступления или мастер скоро первірить</div></div> 
